...
def render_highest_quality(mermaid_code, output_file, max_width=4096):
    ...
    methods = [
        ("PDF+ImageMagick", lambda: render_via_pdf_to_png(mermaid_code, output_file, dpi=600))
    ]
...
def trim_whitespace(image_path):
    """Обрезать белые поля вокруг изображения"""
    try:
        # Обрезка белых полей
        trim_cmd = [
            'convert',
            image_path,
            '-trim',  # Автоматическая обрезка
            '-fuzz', '5%',  # Допуск для белого цвета
            '+repage',  # Сброс страницы
            image_path
        ]
        subprocess.run(trim_cmd, capture_output=True, text=True)
        print(f"✓ Обрезаны белые поля: {image_path}")
        return True
    except Exception as e:
...
graph_name = 'g6'
with open('/Users/maxbig/Coursework_multiprocessing/Coursework/LP/G/g6.txt', "r", encoding="utf-8") as f:
    lines = f.readlines()
...
trim_whitespace(graph_name + '-1.png')
