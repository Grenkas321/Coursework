...
namespace
{
                             ...
                             double &best_score)
    {
        ...
        for (unsigned t = 0; t < trials; ++t)
        {
            ...
            {
                moved = applyRandomLayeredMove(candidate, priority_order, rng) || moved;
            }
            if (!moved)
                continue;
...
namespace scheduling_problem::algorithms
{
    ...
    Schedule SimulatedAnnealing::schedule_(const Graph &graph, const Schedule &base_schedule)
    {
        ...
        for (unsigned restart = 0; restart < restart_count; ++restart)
        {
            ...
            while (true)
            {
                const auto global_iter = iters_count_ + 1;
                const auto current_temp = reduceTemperature_(max_temp_, global_iter);
