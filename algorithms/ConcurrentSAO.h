...
namespace scheduling_problem::algorithms
{
     ...
     *  2) запускает волну параллельных SAO от базового решения,
     *  3) выбирает лучшее и подаёт его в следующую волну,
     *  4) останавливается по saturation и/или rsearch_iters.
     *
     * Seed в стохастических частях детерминированно порождает внутренние seed'ы волн и воркеров.
     * partitions_count задаёт ширину волны, rsearch_iters ограничивает число волн,
     * warm_start переключает базу волны (лучшее текущее или исходный Greedy),
     * subareas включает дополнительную диверсификацию воркеров.
     */
