...
// Целевая функция (аналог goal_function из Python)
int goal_function(const vector<int>& part_sched) {
    int f_hp = 0;
    
    for (int k = 0; k < part_sched.size(); k++) {
        int f_hp_k = 0;
        
        // Суммируем все веса для вершин до k включительно
        for (int i = 0; i <= k; i++) {
        ...
        // Вычитаем веса буферов, у которых все потомки уже обработаны
        for (int i = 0; i < k; i++) {
            int vertex = part_sched[i];
            for (const auto& buf : dct[vertex]) {
                bool all_descendants_scheduled = true;
                for (int desc : buf.second) {
                    // Проверяем, находится ли потомок в уже обработанной части расписания
                    bool found = false;
                    for (int j = 0; j < k; j++) {
                        if (part_sched[j] == desc) {
                            found = true;
                            break;
                        }
                    }
                    if (!found) {
                        all_descendants_scheduled = false;
                        break;
                    }
                }
                if (all_descendants_scheduled) {
                    f_hp_k -= buf.first;
                }
            }
        }
        
        f_hp = max(f_hp, f_hp_k);
    }
...
Schedule Greedy::schedule_(const Graph &graph) {
    ...
    */
    ...
    map<int, set<int>> vertexes_with_ancestors;
    
    // Парсинг входных данных
    for (const string& line : content) {
        vector<string> parts = split(line, ' ');
        
        // Извлекаем номер вершины (первый элемент)
        int cur_v = stoi(parts[0]);
        
        // Инициализируем множество предков для текущей вершины
        if (vertexes_with_ancestors.find(cur_v) == vertexes_with_ancestors.end()) {
            vertexes_with_ancestors[cur_v] = set<int>();
        }
        
        // Объединяем оставшиеся части строки и парсим буферы
        string remaining;
        ...
        vector<string> buffers_str = split(remaining, ',');
        
        for (string& buf_str : buffers_str) {
            buf_str = trim(buf_str);
            vector<string> buf_parts = split(buf_str, ' ');
            
            if (buf_parts.empty()) continue;
            
            // Извлекаем вес буфера (удаляем двоеточие в конце)
            string weight_str = buf_parts[0];
            if (weight_str.back() == ':') {
                weight_str = weight_str.substr(0, weight_str.length() - 1);
            }
            int weight = stoi(weight_str);
            
            // Извлекаем потомков
            set<int> descendants;
            for (int i = 1; i < buf_parts.size(); i++) {
                if (!buf_parts[i].empty()) {
                    int descendant = stoi(buf_parts[i]);
                    descendants.insert(descendant);
                    
                    // Добавляем текущую вершину как предка для потомка
                    if (vertexes_with_ancestors.find(descendant) == vertexes_with_ancestors.end()) {
                        vertexes_with_ancestors[descendant] = set<int>();
                    }
                    vertexes_with_ancestors[descendant].insert(cur_v);
                }
            }
            
            dct[cur_v].push_back(make_pair(weight, descendants));
        }
    }
    
    map<int, set<int>> dct2 = vertexes_with_ancestors; // Копия для дальнейшего использования
    
    // Топологическая сортировка
    vector<int> topo_sort;
    while (topo_sort.size() < content.size()) {
        for (auto it = vertexes_with_ancestors.begin(); it != vertexes_with_ancestors.end(); ) {
            if (it->second.empty()) {
                topo_sort.push_back(it->first);
                int vertex_to_remove = it->first;
                it = vertexes_with_ancestors.erase(it);
                
                // Удаляем эту вершину из множеств предков всех остальных вершин
                for (auto& pair : vertexes_with_ancestors) {
                    pair.second.erase(vertex_to_remove);
                }
                break;
            } else {
                ++it;
    ...
    for (int cur_v : topo_sort) {
        ...
        // Находим позицию для вставки на основе предков
        for (int i = schedule.size() - 1; i >= 0; i--) {
            if (dct2[cur_v].find(schedule[i]) != dct2[cur_v].end()) {
                l_k = i + 1;
                break;
            }
        }
        ...
        // Пробуем вставить вершину на все возможные позиции
        for (int d = l_k; d <= schedule.size(); d++) {
            vector<int> test_schedule = schedule;
            test_schedule.insert(test_schedule.begin() + d, cur_v);
            
            int f_value = goal_function(test_schedule);
            
            if (best_schedule.empty() || f_value < min_f) {
    ...
    vector<int> incr_mas = {0};
    vector<int> decr_mas;
    vector<int> part_sched = schedule;
    ...
    for (int k = 0; k < part_sched.size(); k++) {
        ...
        for (int i = 0; i <= k; i++) {
            int vertex = part_sched[i];
            for (const auto& buf : dct[vertex]) {
                f_hp_k += buf.first; // buf.first - вес буфера
            }
        }
        ...
        for (int i = 0; i < k; i++) {
            int vertex = part_sched[i];
            for (const auto& buf : dct[vertex]) {
                // Проверяем, что все потомки находятся в part_sched[:k]
                if (allElementsInVector(buf.second, part_sched, k)) {
                    f_hp_k -= buf.first;
                    decr += buf.first;
                }
            }
