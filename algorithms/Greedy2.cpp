...
namespace scheduling_problem::algorithms
{
    ...
    Greedy2::Greedy2(const std::string &label)
        : BaseOptimization(label)
    {
    }

    std::unique_ptr<BaseOptimization> Greedy2::copy() const
    {
        return std::unique_ptr<BaseOptimization>(new Greedy2(*this));
    }

    void Greedy2::setParams(const ParamSet &params)
    {
    ...
    ParamSet Greedy2::getParams() const
    {
    ...
    Schedule Greedy2::schedule_(const Graph &graph)
    {
        ...
        while (done < n)
        {
            ...
            for (const auto task : ready)
            {
                ...
                // Получение информации о выходных буферах текущей задачи
                weight_t output_buffers_weight_sum = 0;
                weight_t weighted_duration_sum = 0;

                // Проверяем, что task не выходит за пределы groups
                if (task < groups.size()) {
                    for (const auto& [bid, gs] : groups[task]) {
                        // gs.weight - объем памяти буфера
                        // gs.remaining - количество потребителей (работ, которые используют этот буфер)
                        
                        if (gs.weight > 0) {
                            // Сумма объемов выходных буферов
                            output_buffers_weight_sum += gs.weight;
                            
                            // Для каждого буфера нужно получить суммарную длительность работ-потребителей
                            // Информацию о потребителях можно получить из bindex
                            weight_t consumers_total_duration = 0;
                            
                            // Ищем информацию о буфере в bindex.groups[task]
                            auto it = bindex.groups[task].find(bid);
                            if (it != bindex.groups[task].end()) {
                                const auto& buffer_info = it->second;
                                // buffer_info.consumers - список работ-потребителей этого буфера
                                for (size_t consumer : buffer_info.consumers) {
                                    if (consumer < duration.size()) {
                                        consumers_total_duration += duration[consumer];
                                    }
                                }
                            }
                            
                            // Сумма произведений объема буфера на суммарную длительность потребителей
                            weighted_duration_sum += gs.weight * consumers_total_duration;
                        }
                    }
                }
                
                weight_t kbh = output_buffers_weight_sum * duration[task] + weighted_duration_sum;
                
                /*
                std::cout << "\n  Considering task " << task
                              << " (ready at " << dep_ready << "), kbh = " << kbh << std::endl;
                */
                for (unsigned p = 0; p < pcount; ++p)
                {
                    ...
                    */
                    ...
                    if (!found ||
                        kbh < best_kbh ||
                        (kbh == best_kbh && (finish < best_finish ||
    ...
    size_t Greedy2::choice(const Graph &graph,
    ...
    Greedy2::heuInfo(const Graph &graph, const ScheduleStatus &status, size_t curr_vid)
    {
