...
def define_y(
    ...
    for key, value in sizes.items():
        y_bounds.append("y_" + str(key[0]) + '_' + str(value) + "_" + str(key[1]) + " = 1")
    for k in nodes:
        r_k = []
        rr_kk = []
        ...
        for i in nodes:
            ...
            for j in range(1, bfrs[i][0] + 1):
                r_k.append(str(bfrs[i][j]) + ' y_' + str(i) + '_' + str(j) + '_' + str(k))
                y_binary.append("y_" + str(i) + '_' + str(j) + "_" + str(k))
                if i == k:
                    y_bounds.append("y_" + str(i) + '_' + str(j) + "_" + str(k) + " = 1")
                else:
                    y_subj.append(f's_{k} - s_{i} - {D} y_{i}_{j}_{k} >= {1 - D - int(times[k])}')
                for ii in nodes_cpy:
                    for jj in range(1, bfrs[ii][0] + 1):
                        y_binary.append(f'a_{i}_{j}_{ii}_{jj}_{k}')
                        y_subj.append(f'{4 * D} a_{i}_{j}_{ii}_{jj}_{k} - {D} y_{i}_{j}_{k} - {D} y_{ii}_{jj}_{k} + {D} y_{i}_{j}_{ii} + {D} y_{ii}_{jj}_{i} <= {2 * D + 1}')
                        rr_kk.append(f'{min(int(bfrs[i][j]), int(bfrs[ii][jj]))} a_{i}_{j}_{ii}_{jj}_{k}')
        r_subj.append(' + '.join(r_k) + ' - ' + ' - '.join(rr_kk) + ' <= ' + str(M))
    for k in nodes:
        for key, value in sizes.items():
            i, v, j = key[0], key[1], value
            if i != k and k != v:
                y_subj.append(f'{D} y_{i}_{j}_{k} + {D} z_{i}_{k} + s_{k} - s_{v} >= {int(times[v])}')
